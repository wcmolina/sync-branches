...
// src/constants.ts
var COMMIT_MSG = "Merge '{source}' into {target} [skip ci]";
...
function run() {
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (mergeStatus.failed.length > 0)
        (0, import_core.setFailed)(output);
    } catch (e) {
      const output = generateOutput(mergeStatus);
...
function generateOutput(status) {
  const output = JSON.stringify(status, null, 2);
  (0, import_core.info)(output);
  (0, import_core.setOutput)("MERGE_STATUS", output);
  return output;
}
run();
